<?php
	...
	//更新用户最近登录记录
	$db->update("update p_user set pu_lastlogin=:time where pu_id=:puid", array(
	...
	//--------------------------------- 以下为获取权限数据代码 --------------------------------------------------
	//设置参数
	$param = array(":puid"=>$row['pu_id']);
	
	//查询所属部门信息
	$sql_org = "select p_org.* from p_user, p_org where po_id=pu_oid and pu_id=:puid ";
	$set_org = $db->getone($sql_org, $param);
	
	//查询上下级信息
	// $sql_ = "";
	// $set_ = $db->getlist($sql_, $param);
	...
	$set_group = $db->getlist($sql_group, $param);
	...
	$set_role = $db->getlist($sql_role, $param);
