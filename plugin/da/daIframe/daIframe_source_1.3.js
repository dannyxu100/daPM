...
var daIframe = (function(){
	...
	daIframe.fnStruct = daIframe.prototype = {
		...
		removeCache: function( code ){
			...
			for( var i=0,len=cache.length; i<len; i++ ){
				if( code == cache[i].code ){
					daIframe.clear( cache[i].iframe );		//回收内存
		...
		*/
		goto: function ( url, isNew, code, type ){
			...
			if( !this.isSelfWinCache ) {										//本页面跳转
				daIframe.clear( currentObj.iframe );		//回收内存
			...
			if( null === cacheObj ){											//没有访问过，没有缓存，需要创建一个新的iframe
				newIframe = currentObj.iframe.cloneNode();
		...
		*/
		autoSize: function ( type, winObj ){
			...
			// da(curIframe).data("daIframe_parentwidth", curIframe.parentNode.offsetWidth);
			// da(curIframe).data("daIframe_parentheight", curIframe.parentNode.offsetHeight);
			// da.out("orgWith:"+ da(curIframe).data("daIframe_parentwidth"));
			// da.out("orgHeight:"+ da(curIframe).data("daIframe_parentwidth"));
			
			curIframe.style.width = "100%";
			...
			if( "complete" === curIframe.readyState || curObj.loaded ){				//访问过并已加载完毕
				//da.out(" complete ");
				daIframe.setSize( type, winObj, context, curObj, curIframe ); 
				return; 
			}
	...
	/**为尽量避免iframe内存泄露，专门集中处理的函数
	*/
	daIframe.clear = function( iframe ){
		iframe.src = "about:blank";			//防止内存泄露
		try{
	...
	*/
	...
	/**获得iframe内部页面尺寸，并设置iframe尺寸
	*/
	daIframe.setSize = function( type, winObj, daIframeObj, currentObj, iframe ) {
		...
		try{
			...
			&& "undefined" != typeof iframe.contentDocument.body 
			&& iframe.contentDocument.body.offsetHeight) {
				inWinSize.width = iframe.contentDocument.body.offsetWidth;
				inWinSize.height = iframe.contentDocument.body.offsetHeight;
			  
			}
			else if ( "undefined" != typeof iframe.contentWindow.document 
			&& iframe.contentWindow.document 
			&& iframe.contentWindow.document.body.scrollHeight ) {
				inWinSize.width = iframe.contentWindow.document.body.scrollWidth;
				inWinSize.height = iframe.contentWindow.document.body.scrollHeight;
			  
			}

		}catch(ex){}

		defaultSize.width = iframe.getAttribute( "defaultwidth" ) || "0";
		defaultSize.width = parseInt( defaultSize.width.toLowerCase().replace("px", "") );
		defaultSize.height = iframe.getAttribute( "defaultheight" ) || "0";
		...
		// if( inWinSize.width ){			//宽度暂时使用计算值了
			// iframe.style.width = inWinSize.width + "px";
		// }
		
		if( inWinSize.height ){
			iframe.style.height = ( inWinSize.height + daIframeObj.extendHeight ) + "px";	//danny.xu 2012-2-10 15:44:15 多+30为了迎合豪庭系系统的漂浮菜单；
		}
		
		// da(iframe).data("daIframe_winwidth", inWinSize.width);
		// da(iframe).data("daIframe_winheight", inWinSize.height);
		// da.out("IframeWidth:"+iframe.offsetWidth);
		// da.out("IframeHeight:"+iframe.offsetHeight);
		
		currentObj.loaded = true;					//现场缓存对象 首次加载操作完成
		daIframeObj.reSize( winObj, iframe );		//重算所有缓存iframe的总宽度
