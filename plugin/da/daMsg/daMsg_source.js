...
/*
*/
var daMsg = function( daPtSetting ){
		return new daMsg.fnStruct.init( daPtSetting );
...
daMsg.fnStruct = daMsg.prototype = {
		...
		//daMsg初始化函数
		/*
			daPtSetting: 用户自定义参数列表
		*/
		init: function( daPtSetting ){
			if( !da.isPlainObj( daPtSetting ) ) daPtSetting = { html: daPtSetting };
			
			this.daPtSetting = daPtSetting = da.extend( true, {}, this.daPtSetting, daPtSetting );
			this.daPtParentObj = daPtSetting.parent || doc.body;
		...
		*/
		create: function(){
			var daPtSetting = this.daPtSetting,
					daPtObj = doc.createElement("div");
			
			daPtObj.id = "daMsg_" + this.daPtId;
			daPtObj.className = daPtSetting.css.daMsg;
